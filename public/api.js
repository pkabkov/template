 ...
 */
export async function getTopTagsForArtist(artistName) {
 ...
 *
 * @async
 * @returns {Promise<Object[]>} Массив артистов.
 */
 ...
 *
 * @async
 * @returns {Promise<Object[]>} Массив объектов треков.
 */
 ...
 */
export async function getTopTagsForTrack(artistName, trackName) {
  ...
  try {
    ...
    if (data.track && data.track.toptags && data.track.toptags.tag) {
      return data.track.toptags.tag.slice(0, 3).map(tag => tag.name);
    }
  } catch (error) {
 ...
 */
export async function fetchTrackInfo(artist, track) {
